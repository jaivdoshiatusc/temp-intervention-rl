...
class LunarLanderEvalCallback(BaseCallback):
    ...
    def __init__(self, cfg, eval_env, eval_freq, eval_seed, gif_freq, n_eval_episodes, verbose=1):
        super(LunarLanderEvalCallback, self).__init__(verbose)
    ...
    def create_frame(self):
        ...
        blocker_line = LunarLanderEvalCallback.PLAY_AREA[1] - self.catastrophe_clearance - self.blocker_clearance
        catastrophe_line = LunarLanderEvalCallback.PLAY_AREA[1] - self.catastrophe_clearance
            ...
            while not done:
                ...
                if exp_type in ["ours", "hirl"]:
                    ...
                    if num_timesteps <= blocker_switch_time:
                        ...
                        action_item = 2 if blocker_heuristic_decision else action_item
                    else:
                        episode_env_interventions += 1 if blocker_model_decision else 0
                        episode_exp_interventions += 1 if blocker_heuristic_decision else 0
                        action_item = 2 if blocker_model_decision else action_item
